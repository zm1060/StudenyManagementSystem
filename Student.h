...
#define   STU_NUM 1000                     /* 最多的学生人数 */
#define   COURSE_NUM 3                    /* 最多的考试科目数 */
...
char *courseName[COURSE_NUM] ={"语文","数学","英语"};                  //保存课程名称 
...
STU *CreatEmpty(int n, int m);                          //创建空链表 
STU  *SortbyNumber1(STU *head, int n);                  //按学号由高到低排出成绩表
STU  *SortbyNum(STU *head);                            //按学号由低到高排出成绩表
STU  *SortbyChinese(STU *head, int n);					//按语文成绩由高到低排出成绩表
STU  *SortbyMaths(STU *head, int n);					//按数学成绩由高到低排出成绩表
STU  *SortbyEnglish(STU *head, int n);					//按英语成绩由高到低排出成绩表
STU  *SortbyAver(STU *head, int n);                     //按平均成绩由高到低排出成绩表
...
void  WritetoFile(STU *head, char filename[100], int n, int m); //将每个学生的纪录信息写入文件
STU *ReadfromFile(STU *head, int *n, int *m);                   //从文件读取每个学生的记录信息 
